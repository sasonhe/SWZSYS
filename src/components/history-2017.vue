<template>
  <div class="histy">
    <!-- 无意义code 只为监听 -->
    <input type="hidden" :value="$t('hisYear.yearB')">
            ...
            <van-row type="flex"  style="margin-bottom:.4rem;">
              <van-col :span="span" v-for="(itemArr,idx) in items" :key="idx">
                <div class="item" @click="toggleText(itemArr,itemArr.id)" :class="{ active: itemArr.active }">
                  <div class="top" >
                    <van-image width="100%" :src="'./static/history/2017/'+itemArr.id+'.jpg'"/>
                  </div>
                  <div class="main">
...
<script>
  export default {
    ...
    updated(){
      this.jsonData = this.historyA
      this.forData();
    },
    computed:{
      historyA(){
        return this.$t('historyA')
      }
    }
  }
</script>
